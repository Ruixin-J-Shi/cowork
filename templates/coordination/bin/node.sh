...
case "$CMD" in
  boot)
    ...
    if [ -n "$TAKE" ]; then
      # the slot above was the gate (claim.sh judged the evidence); the lock below belongs to the same dead session, and our own
      # takeover line above has just made "lead activity" look fresh, so the below step is forced by construction
      m="$(bash "$COORD/bin/lead.sh" takeover --reason "$REASON" --desc "node $(node_path): $DESC" --force)" || { echo "slot above taken but the lead lock here could not be — resolve with lead.sh status / takeover before continuing" >&2; exit 1; }
      echo "below: took over the lead lock of $C_REL ($SEATS slots)"
    else
      m="$(bash "$COORD/bin/lead.sh" claim --desc "node $(node_path): $DESC")" || { echo "lead lock here is held — standby or takeover per TREE.md" >&2; exit 2; }
